...
$(document).ready(function () {
    ...
    if (enteredMovie) {
      logMovie(enteredMovie)
        .then(function (movieGenre) {
          //show results container by removing hidden class
          $(".search-results").removeClass("hidden");

          if (movieGenre && movieGenre.trim() !== "") {
            $(".actual-results").removeClass("hidden");
            $(".error-message").addClass("hidden");
            return getMealsuggestionsByGenre(movieGenre);
          } else {
            $(".actual-results").addClass("hidden");
  ...
  function logMovie(enteredMovie) {
    ...
    return fetch(
      `https://www.omdbapi.com/?t=${encodedMovie}&apikey=${omdbApiKey}`
    )
  ...
  function logRecipe(cuisine) {
    ...
    fetch(
      `https://api.edamam.com/search?q=${cuisine}&app_id=${edamamApiId}&app_key=${edamamApiKey}&to=4`
    )
      .then(function (response) {
        return response.json();
      })
